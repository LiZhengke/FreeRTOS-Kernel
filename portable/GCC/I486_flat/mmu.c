...
void create_user_page_directory(uint32_t* pgd_phys, uint32_t** pgd_virt) {
    ...
    pd[0] = page_directory[0]; /* Identity map the first 4MB for user-space code */
...
extern char _kernel_phys_end[];     /* Physical start point (for example 0x150000). */
extern char _user_text_vma_end[];   /* User text VMA end symbol. */
